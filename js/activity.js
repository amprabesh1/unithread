...
(function () {
  ...
  function render() {
    ...
    container.innerHTML = `
      <h2 class="text-xl font-bold text-neutral-800 mb-4">My Activity</h2>
      ...
      <div id="activity-posts" class="space-y-4 ${tab !== 'posts' ? 'hidden' : ''}">
        ${posts.length ? posts.map((p, i) => `
          <div class="app-card p-4 flex flex-wrap items-center justify-between gap-3 fade-in-up" style="animation-delay: ${i * 50}ms;">
            <div>
      ...
      <div id="activity-requests" class="space-y-4 ${tab !== 'requests' ? 'hidden' : ''}">
        ${myRequests.length ? myRequests.map((r, i) => {
          ...
          return `
            <div class="app-card p-4 flex flex-wrap items-center justify-between gap-3 fade-in-up" style="animation-delay: ${i * 50}ms;">
              <div>
                <span class="text-neutral-800">${post ? (post.type === 'ride' ? 'Ride: ' + (post.destination || '—') : post.type === 'task' ? 'Task: ' + (post.description || '—').slice(0, 40) : 'Maintenance: ' + (post.location || '—')) : 'Post'}</span>
      ...
      <div id="activity-offers" class="space-y-4 ${tab !== 'offers' ? 'hidden' : ''}">
        ${offersReceived.length ? offersReceived.map((r, i) => {
          ...
          return `
            <div class="app-card p-4 fade-in-up" style="animation-delay: ${i * 50}ms;">
              <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
                <span class="font-medium text-neutral-800">${fromUser.displayName}</span>
                <span class="text-sm text-neutral-500">${formatDate(r.createdAt)}</span>
              </div>
              <p class="text-sm text-neutral-600 mb-3">${post ? (post.type === 'ride' ? 'Ride to ' + (post.destination || '—') : 'Task: ' + (post.description || '—').slice(0, 60)) : ''}</p>
              ${r.status === 'pending' ? `
                <div class="flex gap-2">
                  ...
                  <button type="button" class="activity-decline px-3 py-1.5 rounded-lg text-sm font-medium text-neutral-600 bg-neutral-100 hover:bg-neutral-200" data-req-id="${r.id}">Decline</button>
                </div>
