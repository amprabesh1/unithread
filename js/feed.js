...
(function () {
  ...
  function primaryAction(post, currentUserId) {
    ...
    if (post.type === 'ride') {
      ...
      return '<button type="button" class="post-action-request-join btn-action btn-action-ride text-neutral-700" data-post-id="' + post.id + '">Request to Join</button>';
    }
    if (post.type === 'task') {
      if (isAuthor) return '';
      if (post.status !== 'open') return '';
      if (alreadyRequested) return '<span class="text-sm text-neutral-500">Offer sent</span>';
      return '<button type="button" class="post-action-offer-help btn-action btn-action-task text-neutral-700" data-post-id="' + post.id + '">Offer Help</button>';
    }
    if (post.type === 'maintenance') {
  ...
  function renderCard(post, currentUserId, index) {
    ...
    return `
      <article class="post-card app-card p-4 md:p-5 fade-in-up delay-${delay} bg-white dark:bg-neutral-800 border border-transparent dark:border-neutral-700" data-post-id="${post.id}" style="animation-fill-mode: both;">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2 min-w-0">
                <span class="font-semibold text-neutral-800 text-[15px]">${(author.displayName || 'Unknown').replace(/</g, '&lt;')}</span>
                ${verifiedBadge}
            ...
            <h3 class="font-bold text-neutral-800 text-base mt-2 leading-snug">${(cardTitle(post)).replace(/</g, '&lt;')}</h3>
    ...
    container.innerHTML = `
      ...
      <div class="space-y-4" id="feed-cards">
        ...
        <div class="empty-state fade-in-up flex flex-col items-center justify-center py-12 px-4 text-center">
          ${emptyStateSVG()}
          <h3 class="text-xl font-bold text-neutral-800 mt-6">Your campus feed is quiet... for now</h3>
