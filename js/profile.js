    ...
    container.innerHTML = `
      <div class="app-card p-6 md:p-8 fade-in-up delay-0 bg-white dark:bg-neutral-800">
        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          <div class="avatar-initials w-24 h-24 rounded-full flex-shrink-0 text-2xl bg-neutral-200 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-200">
            ${user.photoUrl ? `<img src="${user.photoUrl}" alt="" class="w-full h-full rounded-full object-cover" />` : initials}
          </div>
          <div class="flex-1 text-center sm:text-left">
            <h2 class="text-xl font-bold text-neutral-800 dark:text-white">${(user.displayName || 'Unknown').replace(/</g, '&lt;')}</h2>
            <p class="text-neutral-600 dark:text-neutral-400 mt-1">${(user.email || '').replace(/</g, '&lt;')}</p>
